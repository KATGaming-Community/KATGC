...
depotTruck = "OPTRE_m1087_stallion_unsc_resupply" createVehicle getMarkerPos "FOB_Depot";
...
while {(alive depotTruck) && (depotTruck distance (getMarkerPos "FOB_Marker") > 20) && (Fob_missions)} do {
	...
	if ((_targetPos distance (getMarkerPos "respawn_west")) > 1500 && (_targetPos distance (getMarkerPos "FOB_Marker")) > 100) then { 
		sleep 1;
		_GRP1 = [_targetPos, EAST, (configfile >> "CfgGroups" >> "Indep" >> "IND_C_F" >> "Infantry" >> "BanditCombatGroup")] call BIS_fnc_spawnGroup;
