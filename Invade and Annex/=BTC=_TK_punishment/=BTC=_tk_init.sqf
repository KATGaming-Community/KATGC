...
BTC_Teamkill =
{
	...
	switch (true) do
	{
		...
		{
			titleText ["STOP TEAMKILLING WAIT 30 seconds!","BLACK FADED", 30];
			sleep 30;
			titleText ["STOP TEAMKILLING OR YOU WILL BE BANNED!","PLAIN", 50];
			failMission "end1";
		};
		...
		case (BTC_teamkiller > BTC_tk_last_warning) : 
		{
			...
			disableUserInput true;
			titleText ["YOU HAVE BEEN PUNISHED FOR TEAMKILLING AND YOU ARE NOT WELCOME ANYMORE","BLACK FADED"];
			player setPos [0,0,0];
			while {true} do
			{
				titleText ["YOU VE BEEN PUNISHED FOR TEAMKILLING AND YOU ARE NOT WELCOME ANYMORE","BLACK FADED"];
				sleep 1;
			};
			disableUserInput false;
		};
