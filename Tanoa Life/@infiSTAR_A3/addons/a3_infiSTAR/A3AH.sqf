...
UPDATEEMAIL='colishere@gmail.com';
INFISTARVERSION='16-05-2017 06-11-07 - v0249';
...
if(!_return)exitWith
{
	diag_log format['<infiSTAR.de> %1 - serverCommandPassword NOT SET!    colishere@gmail.com - 16-05-2017 06-11-07 - v0249 - %2 - %3',_t,serverName,productVersion];
...
_s_m = toArray "colishere@gmail.com";_s_u = toArray "PUT YOUR STEAM64 UID HERE!";_s_v = toArray "16-05-2017 06-11-07 - v0249";_s_s = toArray servername;
...
{
	diag_log format['<infiSTAR.de> %1 - infiSTAR_AdminMenu.hpp is not in your MPMission or you forgot to #include it within your description.ext   [colishere@gmail.com - 16-05-2017 06-11-07 - v0249 - %2 - %3]',_t,serverName,productVersion];
};
_test = [0,{}] execFSM 'call.fsm';
_test = [0,{}] execFSM 'call.fsm';
if(_test isEqualTo 0)exitWith
{
	_log = format['<infiSTAR.de> %1 - call.fsm missing in your MPmission!    colishere@gmail.com - 16-05-2017 06-11-07 - v0249 - %2 - %3',_t,serverName,productVersion];
...
FNC_A3_CUSTOMLOG = compileFinal "
	...
	diag_log format['<infiSTAR.de>%1| %2   [16-05-2017 06-11-07 - v0249]',_logname,_logentry];
...
_fnc_RandomGen =
{
	_fnc_actualGen = {
		...
		_arr = ['a','f','1','d','1','f','3','2','f','5','d','c','1','0','5','3','d','9','c','e','f','e','1','e','1','9','0','f','9','6','f','1','9'];
...
_FNC_AHe8dbd04db812_KICKLOG = call _fnc_RandomGen;['RANDOMVAR',format['_FNC_AHe8dbd04db812_KICKLOG: %1',_FNC_AHe8dbd04db812_KICKLOG]] call FNC_A3_CUSTOMLOG;
_FNC_AHe8dbd04db812_KICKLOGSPAWN = call _fnc_RandomGen;['RANDOMVAR',format['_FNC_AHe8dbd04db812_KICKLOGSPAWN: %1',_FNC_AHe8dbd04db812_KICKLOGSPAWN]] call FNC_A3_CUSTOMLOG;
...
_AHe8dbd04db812KICKLOG = call _fnc_RandomGen;['RANDOMVAR',format['_AHe8dbd04db812KICKLOG: %1',_AHe8dbd04db812KICKLOG]] call FNC_A3_CUSTOMLOG;
...
_AHe8dbd04db812_KICKLOG = call _fnc_RandomGen;['RANDOMVAR',format['_AHe8dbd04db812_KICKLOG: %1',_AHe8dbd04db812_KICKLOG]] call FNC_A3_CUSTOMLOG;
...
_fnc_infiSTAR_PlayerLog = {
	...
	{
		_log = 'BadName (KICKED)';
		[_name,_uid,'SLOG_SKICK',toArray(_log)] call "+_FNC_AHe8dbd04db812_KICKLOG+";
	};
						...
						"; if(_UVC)then{ _A3AHstring = _A3AHstring + "
							if((!("+str _UVW+") && {_vehstype in "+str _ForbiddenVehicles+"}) || (("+str _UVW+") && {!(_vehstype in "+str _VehicleWhiteList+")}))then
							{
								...
								if(_crew isEqualTo [])then
								{
									...
									['SERVER','SERVER','HLOG',toArray(_log)] call "+_FNC_AHe8dbd04db812_KICKLOG+";
								...
								else
								{
									...
									_log = format['BadVehicle (S-UVC+LVC): %1',_vehstype];
									{
										_name = name _x;
										_puid = getPlayerUID _x;
										[_name,_puid,'HLOG',toArray(_log)] call "+_FNC_AHe8dbd04db812_KICKLOG+";
									} forEach _crew;
									_veh call fnc_deleteObject;
								};
							};
					...
					if(!isNull _veh)then
					{
						if((_plrObj == _veh) && (_vehstype != _vehptype))then
						{
							...
							if!(_veh in _vehicles)then
							{
								...
								[_name,_puid,'SLOG',toArray(_log)] call "+_FNC_AHe8dbd04db812_KICKLOG+";
							};
						};
					};
				};
...
"+_FNC_AHe8dbd04db812_KICKLOG+" = compileFinal ([_FNC_AHe8dbd04db812_KICKLOG] call fnc_CompilableString);
...
_FNC_AHe8dbd04db812_KICKLOGSPAWN =
{
	...
	if(isNil '_input')exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + 'FNC_AHe8dbd04db812_KICKLOGSPAWN _this is Nil (v0249)';
		_log call FNC_A3_SURVEILLANCELOG;
	};
	diag_log format['<infiSTAR.de FNC_AHe8dbd04db812_KICKLOGSPAWN> %1',_this];
	...
	diag_log format['<infiSTAR.de FNC_AHe8dbd04db812_KICKLOGSPAWN> %1(%2) Token %3 - objByNetID %4',_name,_puid,_tokenreceived,_objectFromNetId];
	...
	if!(_puid isEqualTo _uid_by_token)exitWith
	{
		...
		diag_log format['<infiSTAR.de FNC_AHe8dbd04db812_KICKLOGSPAWN> %1',_log];
		[_name,_puid,'SLOG_SKICK',toArray(_log)] call "+_FNC_AHe8dbd04db812_KICKLOG+";
		[_name,_uid_by_token,'SLOG_SKICK',toArray(_log)] call "+_FNC_AHe8dbd04db812_KICKLOG+";
	};


	_arraysent set [0,_objectName];
	_arraysent set [1,_puid];
	_arraysent call "+_FNC_AHe8dbd04db812_KICKLOG+";
};
"+_FNC_AHe8dbd04db812_KICKLOGSPAWN+" = compileFinal ([_FNC_AHe8dbd04db812_KICKLOGSPAWN] call fnc_CompilableString);
...
fn_adminConnected_infiSTAR = {
	...
	if(_uid in _admins)then
	{
		...
		_fnc_adminLog = {};
		"; if(_LogAdminActions)then{ _A3AHstring = _A3AHstring + "
			if(_uid in _locDevs)then{_fnc_adminLog = {};}else{diag_log format['<infiSTAR.de> ******ADMIN-LOGIN******: %1(%2) (v0249)',_name,_uid];_fnc_adminLog = compile(format['[''%1'',''%2'',''ALOG'',toArray _this] call "+_AHe8dbd04db812KICKLOG+"',_name,_uid]);};
		"; }; _A3AHstring = _A3AHstring + "
...
_AH_MAIN_BLOCK = {
	...
	if(isNil'"+_AHe8dbd04db812KICKLOG+"')exitWith{
	...
	if!(_puid isEqualTo _clientUID)exitWith
	{
		...
		[_name,_puid,'SLOG_SKICK',toArray(_log)] call "+_AHe8dbd04db812KICKLOG+";
		(findDisplay 46)closeDisplay 0;
	};
	...
	if!(_admin)then
	{
		...
		if(_add_Id > -1)then
		{
			...
			if(_announce)then
			{
				...
				[profileName,_puid,'HLOG_SKICK',toArray(_log)] call "+_AHe8dbd04db812KICKLOG+";
				[] call "+_AHKickOFF+";
			};
		};

		{
			if(!isNil _x)then
			{
				...
				[profileName,_puid,'SLOG_SKICK',toArray(_log)] call "+_AHe8dbd04db812KICKLOG+";
				(findDisplay 46)closeDisplay 0;
			};
		} forEach [
	...
	'a3_m3editor','extdb2','loki_lost_key','infistar_lifemods','infistar_a3','balca_debug_tool','nss_admin_console','sspcm'
		...
		_cfgPatches = 'true' configClasses (configFile >> 'CfgPatches');
		{
			...
			if(_lconfigName in _badclassesban)then
			{
				...
				[_name,_puid,'BAN',toArray(_log)] call "+_AHe8dbd04db812KICKLOG+";
				[] call "+_AHKickOFF+";
			};
			if(_lconfigName in _badclasseskick)then
			{
				_log = format['Bad Addon found in CfgPatches: %1',_configName];
				[_name,_puid,'HLOG_SKICK',toArray(_log)] call "+_AHe8dbd04db812KICKLOG+";
				[] call "+_AHKickOFF+";
			};
		} forEach _cfgPatches;
...
"; if(_armalog)then{ _A3AHstring = _A3AHstring + "
	[_name,_puid,"+_AHe8dbd04db812KICKLOG+"] spawn {
		params[['_name',''],['_uid',''],['_AHe8dbd04db812KICKLOG',{}]];
		if(!hasInterface)exitWith{};
		waitUntil{getClientStateNumber >= 10};
		waitUntil {!(getPlayerUID player isEqualTo '')};
		uiSleep 10;
		[_name,_uid,'CB',toArray(profileNameSteam)] call _AHe8dbd04db812KICKLOG;
	};
"; }; _A3AHstring = _A3AHstring + "
			...
			_fadeOut = {
				...
				if(!isNull findDisplay 602)then
				{
					...
					if(time > _lastCall)then
					{
						...
						[_name,_puid,'HLOG',toArray(_log)] call "+_AHe8dbd04db812KICKLOG+";
					};
				};
			};
			_dis_head = 0.20;
...
if(_admin)then
{
	if(_ltxt isEqualTo ''!admin'')exitWith
	{
		...
		[_name,_puid,''AC'',toArray(_txt)] call "+_AHe8dbd04db812KICKLOG+";
...
"; if(_VDN)then{ _A3AHstring = _A3AHstring + "
	if(_ltxt in [''!vote day'',''/vote day'',''vote day'',''!day'',''/day''])then{
		(findDisplay 24) closeDisplay 0;
		[_name,_puid,''VOTE'',toArray ''DAY''] call "+_AHe8dbd04db812KICKLOG+";
	};
	if(_ltxt in [''!vote night'',''/vote night'',''vote night'',''!night'',''/night''])then{
		(findDisplay 24) closeDisplay 0;
		[_name,_puid,''VOTE'',toArray ''NIGHT''] call "+_AHe8dbd04db812KICKLOG+";
	};
"; }; _A3AHstring = _A3AHstring + "
false
';
_chatKeyUp_id = -1;
		...
		while{1==1}do
		{
						...
						if(_shape isEqualTo 'POLYLINE')then
						{
							...
							if!(_puid isEqualTo '')then
							{
								...
								if(_puid isEqualTo _varuid)then
								{
									...
									if(_need_hlog)then
									{
										[_name,_varuid,'SLOG',toArray(_log)] call "+_AHe8dbd04db812KICKLOG+";
									};
									_need_hlog = false;
								};
							};
			...
			if(_combatTime <= 30)then
			{
				...
				{
					_inCombat = 1;
					[_name,_puid,'COMBATCHECK',_inCombat] call "+_AHe8dbd04db812KICKLOG+";
				};
			}
			else
			{
...
if(!isNull findDisplay 24 && !isNull findDisplay 49)then
{
	...
	if!(_ctrltext isEqualTo '')then
	{
		...
		[_name,_puid,'SLOG_RE',toArray(_log)] call "+_AHe8dbd04db812KICKLOG+";
	};
	(findDisplay 24) closeDisplay 0;
									...
									{
										{
											if!((toLower _var) find _x isEqualTo -1)then
											{
												...
												[profileName,getPlayerUID player,'HLOG_SKICK',toArray(_log)] call "+_AHe8dbd04db812KICKLOG+";
												[] call "+_AHKickOFF+";
												...
												"; if(_UBV2C)then{ _A3AHstring = _A3AHstring + "
													...
													if(!isNil '_m')then
													{
														...
														if((toLower _mstring) find _x != -1)then
														{
															...
															[profileName,getPlayerUID player,'SLOG',toArray(_log)] call "+_AHe8dbd04db812KICKLOG+";
															(findDisplay 46)closeDisplay 0;
															missionNamespace setVariable[_var,nil];
														};
													};
		...
		[_name,_puid] spawn {
			...
			_FiredCheck = {
				...
				if(("+str _UFA+") && {((!("+str _UAW+") && {_ammo in "+str _ForbiddenAmmo+"}) || (("+str _UAW+") && {!(_ammo in "+str _AmmoWhiteList+")}))})then
				{
					...
					[profileName,getPlayerUID _unit,'BAN',toArray(_log)] call "+_AHe8dbd04db812KICKLOG+";
					[] call "+_AHKickOFF+";
				};
								...
								if(_maxAmmo > 1)then
								{
									...
									if(diag_tickTime - "+_lastshotmade+" > 0.2)then
									{
										...
										if((_camm >= _maxAmmo)&&(_muzzle isEqualTo currentWeapon _unit))then
										{
											...
											[profileName,getPlayerUID _unit,'SLOG',toArray(_log)] call "+_AHe8dbd04db812KICKLOG+";
				...
				if((!isNull (findDisplay 12))&&(visiblemap))then
				{
					...
					if(!_loggedOnce)then
					{
						...
						if(_size > 5)then
						{
							...
							[_name,_puid,'SLOG',toArray(_log)] call "+_AHe8dbd04db812KICKLOG+";
							_loggedOnce = true;
							lbClear _control2;
						};
					};
						...
						if((isNull _driver)||{(player isEqualTo _driver)})then
						{
							if(!isNil '"+_AHpos+"')then
							{
								...
								if(_tmpAHpos isEqualTo [])then
								{
									...
									if(_tpcount > 3)then
									{
										[_name,_puid,'TMPBAN',toArray(_log)] call "+_AHe8dbd04db812KICKLOG+";
										[] call "+_AHKickOFF+";
									}
									else
									{
								...
								else
								{
									...
									[_name,_puid,'TPLOG',toArray(_log),_tmpAHpos select 0,_tmpAHpos select 1,_lastpos,_curpos] call "+_AHe8dbd04db812KICKLOG+";
								};
							}
							else
							{
...
"; }; _A3AHstring = _A3AHstring + "
				...
				"; if(_CLM)then{ _A3AHstring = _A3AHstring + "
					...
					_MCCODE =
					{
						...
						if(str _foundbad != '[]')then
						{
							...
							[profileName,getPlayerUID player,'BAN',toArray(_log)] call "+_AHe8dbd04db812KICKLOG+";
							[] call "+_AHKickOFF+";
						};
					};
		...
		[_name,_puid,_admins] spawn {
					...
					[] spawn {
						...
						if(time - "+_clickOnMapTimer+" > 5)then
						{
							...
							if("+_clickOnMapCaught+" >= 3)then
							{
								...
								[profileName,getPlayerUID player,''SLOG_SKICK'',toArray(_log)] call "+_AHe8dbd04db812KICKLOG+";
							};
						};
					};
				';
			...
			while{1==1}do
			{
				...
				if(!isNull player)then
				{
					...
					"; if(_LVC)then{ _A3AHstring = _A3AHstring + "
						...
						if(!(player isEqualTo _veh) && !(_vehptype isEqualTo (typeOf _veh)))then
						{
							...
							{
								[_name,_puid,'LVC',player,_vehptype,mapGridPosition _veh] call "+_AHe8dbd04db812KICKLOG+";
							};
						};
					"; }; _A3AHstring = _A3AHstring + "
				};
				...
				"; if(_MOD == 'Epoch')then{ _A3AHstring = _A3AHstring + "
					...
					{
						[_name,_puid,'BAN',toArray(format['Altis Life Hack: life_fnc_MPexec [%1] should be [%2]',life_fnc_MPexec,_rnd4])] call "+_AHe8dbd04db812KICKLOG+";[] call "+_AHKickOFF+";
					};
					if!(_rnd5 isEqualTo life_atmbank)then
					{
						[_name,_puid,'BAN',toArray(format['Altis Life Hack: life_atmbank [%1] should be [%2]',life_atmbank,_rnd5])] call "+_AHe8dbd04db812KICKLOG+";[] call "+_AHKickOFF+";
					};
	...
	_hours = floor(serverTime / 60 / 60);_value = ((serverTime / 60 / 60) - _hours);if(_value == 0)then{_value = 0.0001;};_minutes = round(_value * 60);_seconds = '12166';
	_devLog = format['<infiSTAR.de> %1 VERSION: 16-05-2017 06-11-07 infiSTAR.de AHAT (v0249) - server running: %2:%3:%4',VERSIONCHECKRESULT,_hours,_minutes,_seconds];diag_log _devLog;
...
_from = 'colishere@gmail.com';
_vers = '16-05-2017 06-11-07 - v0249';
...
_res = [VERSION_CHECK_URL_FULL] call FN_ARMA_FETCHDATA;
	...
	{
		{
			...
			{
				_name = name _x;
				[_name,_puid,'BAN',toArray('')] call "+_FNC_AHe8dbd04db812_KICKLOG+";
			};
		} forEach allPlayers;
...
_DO_THIS_MORE_OFTEN = {
	...
	if(hasInterFace)then
	{
		...
		} forEach ['"+_AHe8dbd04db812_KICKLOG+"','"+_adminsA+"','"+_MC+"'];
		{
			if(!isNil _x)then
			{
				_var = missionNamespace getVariable _x;
...
while{1==1}do
{
	if(time > _timer0)then
	{
		...
		'"+_AHe8dbd04db812_KICKLOG+"' addPublicVariableEventHandler {(_this select 1) call "+_FNC_AHe8dbd04db812_KICKLOGSPAWN+";};
					...
					if(_veh getVariable ['"+_vehicle_needs_check+"',true])then
					{
						...
						if((!("+str _UVW+") && {_type in "+str _ForbiddenVehicles+"}) || (("+str _UVW+") && {!(_type in "+str _VehicleWhiteList+")}))then
						{
							...
							_log = format['BadVehicle (S-UVC): %1',_type];
							{
								_xname = name _x;
								_xuid = getPlayerUID _x;
								[_xname,_xuid,'HLOG',toArray(_log)] call "+_FNC_AHe8dbd04db812_KICKLOG+";
							} forEach _crew;
							_veh call fnc_deleteObject;
						};
					};
...
_zeroCode = {
	if(hasInterface)then
	{
		_AHKickOFF = compileFinal '[] spawn {scriptName ''KICKOFF'';sleep 0.1;(findDisplay 46)closeDisplay 0;if(!isNil''"+_AHe8dbd04db812KICKLOG+"'')then{[profileName,getPlayerUID player,''KICKME''] call "+_AHe8dbd04db812KICKLOG+";};};';
		if(isNil'"+_AHKickOFF+"')then{"+_AHKickOFF+" = _AHKickOFF;};
		if(str _AHKickOFF != str "+_AHKickOFF+")then{[] call _AHKickOFF;};
		_AHe8dbd04db812KICKLOG = compileFinal ""
			if(!isNil'"+_YourPlayerToken+"')then
			{
				"+_AHe8dbd04db812_KICKLOG+" = ["+_YourPlayerToken+",_this,netId player];publicVariableServer '"+_AHe8dbd04db812_KICKLOG+"';"+_AHe8dbd04db812_KICKLOG+"=nil;
			}
			else
			{
				_this spawn {
					...
					if(isNil'"+_YourPlayerToken+"')exitWith{KICKED_FOR_NIL_TOKEN_AHe8dbd04db812KICKLOG = format['PLAYER__%1__%2',getPlayerUID player,_this];publicVariableServer 'KICKED_FOR_NIL_TOKEN_AHe8dbd04db812KICKLOG';(findDisplay 46)closeDisplay 0;};
					"+_AHe8dbd04db812_KICKLOG+" = ["+_YourPlayerToken+",_this,netId player];publicVariableServer '"+_AHe8dbd04db812_KICKLOG+"';"+_AHe8dbd04db812_KICKLOG+"=nil;
				};
			};
		"";
		if(isNil'"+_AHe8dbd04db812KICKLOG+"')then{"+_AHe8dbd04db812KICKLOG+" = _AHe8dbd04db812KICKLOG;};
		if(str _AHe8dbd04db812KICKLOG != str "+_AHe8dbd04db812KICKLOG+")then{[] call _AHKickOFF;};
