...
if (_spawnPoints isEqualType []) then {
    ...
    {if ((nearestObjects[(getMarkerPos _x),["Car","Ship","Air","Armored"],5]) isEqualTo []) exitWith {_spawnPoint = _x};} forEach _spawnPoints;
} else {
    if ((nearestObjects[(getMarkerPos _spawnPoints),["Car","Ship","Air","Armored"],5]) isEqualTo []) exitWith {_spawnPoint = _spawnPoints};
};
