...
if (((typeOf _curObject) in [] || str _curObject find ": b_sugarcane" > 0 ) && !life_action_gathering && playerSide isEqualTo civilian) exitWith
	{
...
/* FIX FOR NOT BEING ABLE TO LOOK AT ANIMALS TO GUT THEM */
if (_curObject in LIFE_SETTINGS(getArray,"animaltypes_hunting") && (!alive _curObject) && !isNull _curObject && playerSide isEqualTo civilian) exitWith {
    [_curObject] call life_fnc_gutAnimal;
};
...
if ((player distance (getMarkerPos "logging_zone") < 300 || player distance (getMarkerPos "logging_zone1") < 300) && str cursorObject find ": t_" > 0 && player distance cursorObject < 10) exitWith  
{
	if(life_action_gathering) exitWith {};
	life_action_gathering = true;
	_handle = [] spawn life_fnc_gather;
