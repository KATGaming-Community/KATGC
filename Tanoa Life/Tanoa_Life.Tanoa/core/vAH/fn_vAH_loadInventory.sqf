	...
	} else {
{
	...
	switch (_type) do {
		case "imenu": {
			...
			_time = round (_time / 3);
			if (_time <= 0) then {_price = 0; if (EQUAL(_status,1)) then {_toDel = round(_time + 216); _timeDisplay = format[": Removed in %1 Hrs",_toDel]} else {_timeDisplay = "";};} else {_timeDisplay = format[": Expires in %1 Hrs",_time]};
