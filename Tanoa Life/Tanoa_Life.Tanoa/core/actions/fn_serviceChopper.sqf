...
private _positionObject = [_this,3,objNull,[ObjNull]] call BIS_FNC_PARAM;
if (isNull _position) exitWith {};

_serviceCost = LIFE_SETTINGS(getNumber,"service_chopper");
_search = nearestObjects[getPos _positionObject, ["Air"],10];
...
if (!alive (_search select 0) || (_search select 0) distance _positionObject > 15) exitWith {life_action_inUse = false; hint localize "STR_Service_Chopper_Missing"};
