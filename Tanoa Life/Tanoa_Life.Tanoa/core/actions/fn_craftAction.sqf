...
*/
private["_xpNeed","_boat","_dir","_name","_xp","_time","_data","_addExpAmount","_profName","_dialog","_exit","_item","_itemInfo","_oldItem","_newItem","_upp","_itemName","_ui","_progress","_pgText","_cP","_allMaterial","_matsNeed","_invSize","_handledItem","_itemFilter","_backpackOldItems","_weight","_vehspawn","_vehicle"];
...
if(_itemFilter == "vehicle" && (count(nearestObjects[_vehspawn,["Car","Ship","Air","Motorcycle","Armored"],10]) > 0)) exitWith {hint localize "STR_Shop_Veh_Block"};
...
if(_itemFilter == "weapon" && currentWeapon player != "") then 
{
	if (player canAdd _item) exitWith {}; 	
	_full = true;
};
if (_full) exitWith {hint localize "STR_NOTF_NoRoom";};
...
if(!_removeItemSuccess) exitWith {5 cutText ["","PLAIN"]; life_is_processing = false;[] call life_fnc_hudUpdate;};
