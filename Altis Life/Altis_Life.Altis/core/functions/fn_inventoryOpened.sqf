...
if (_isPack isEqualTo 1) exitWith {
    ["STR_MISC_Backpack",true,"slow"] call life_fnc_notification_system;
    true;
};
...
if ((typeOf _container) in ["Box_IND_Grenades_F","B_supplyCrate_F"]) exitWith {
    ...
    if (!(_house in life_vehicles) && (_house getVariable ["locked",true])) exitWith {
       ["STR_House_ContainerDeny",true,"slow"] call life_fnc_notification_system;
        true;
    };
};
...
if (KINDOF_ARRAY(_container,_list)) exitWith {
    if (!(_container in life_vehicles) && (_container getVariable ["locked",true])) exitWith {
        ["STR_MISC_VehInventory",true,"slow"] call life_fnc_notification_system;
        true;
    };
};
...
if (_container isKindOf "Man" && !alive _container) exitWith {
    ["STR_NOTF_NoLootingPerson",true,"slow"] call life_fnc_notification_system;
    true;
};
